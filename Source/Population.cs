...
namespace Genetic_Algorithm.Source
{
    public class Population
    {
        ...
        {
            map = new Map(mapSizeX, mapSizeY, path); // Initialize the map
            ...
            int genSize = (mapSizeX + mapSizeY) * 4; // Calculate the size of the genetic code
        ...
        public void Evaluate(Options options)
        {
            ...
            foreach (Specimen s in speciments)
            {
                ...
                bool[][] visited = new bool[map.mapBoundries.x][]; // Create a visited array
                for (int i = 0; i < map.mapBoundries.x; i++)
                {
